...
import { ChevronDown, ChevronUp, FileSearch, Radar, Sparkles } from "lucide-react";
...
export function MatchWorkspace() {
  ...
  useEffect(() => {
    void fetch(`${API_BASE}/api/presets`, { cache: "no-store" })
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      formData.append(
        "job_ids",
        JSON.stringify(jobs.map((item) => item.job.id)),
      );
            ...
            >
              <Stack spacing={1.5}>
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  ...
                  <Chip label={`默认匹配全部 ${jobs.length} 条`} size="small" color="primary" />
                </Stack>
                <Typography variant="body2" sx={{ color: "var(--muted)", lineHeight: 1.8 }}>
                  为了减少操作成本，这个页面现在不再让用户手动勾选岗位。上传简历后会直接和当前岗位库中的全部岗位进行比较，并输出完整排序结果。
                </Typography>
                                ...
                                <Stack spacing={2}>
                                  ...
                                  {item.dimension_scores.map((dimension) => (
                                    <Box key={dimension.name}>
                                      ...
                                      <LinearProgress
                                        variant="determinate"
                                        value={Math.min((dimension.score / 35) * 100, 100)}
                                  ...
                                  <div className="grid gap-3 md:grid-cols-[1.1fr_0.9fr]">
                                    ...
                                    >
                                      ...
                                      <Typography variant="body2" sx={{ mt: 1, color: "var(--ink)", lineHeight: 1.8 }}>
                                        {item.explanation_details.action_recommendation}
                                      </Typography>
                                    </Paper>
                                  </div>
                                </Stack>
